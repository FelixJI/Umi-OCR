        ...
        if cuda_support:
            ...
            if gpu_count > 0:
                for i in range(gpu_count):
                    ...
                    print(f"   GPU {i}: {props.name}, 显存 {props.total_memory // (1024**3)}GB")
...
def test_ocr_recognition(ocr):
    ...
    try:
        ...
        # 检查识别准确性
        if "Umi" in recognized_text or "测试" in recognized_text or "2026" in recognized_text:
            print("✅ 识别结果包含关键词")
            return True
        else:
            print("⚠️ 识别结果可能不准确")
...
def test_project_imports():
    ...
    try:
        ...
        print(f"✅ utils.dependency_installer 导入成功")
        print(f"   配置版本: PaddlePaddle={PADDLEPADDLE_VERSION}, PaddleOCR={PADDLEOCR_VERSION}")
...
def main():
    ...
    if all_passed:
        ...
        if not results.get("paddlepaddle"):
            print("\n修复建议 - PaddlePaddle:")
            print("   CPU版: pip install paddlepaddle==3.3.0 -i https://www.paddlepaddle.org.cn/packages/stable/cpu/")
            print("   GPU版: pip install paddlepaddle-gpu==3.3.0 -i https://www.paddlepaddle.org.cn/packages/stable/cu118/")
