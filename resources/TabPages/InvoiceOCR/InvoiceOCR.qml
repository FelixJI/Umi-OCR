...
TabPage {
    ...
    function onStart(count) {
        console.log("发票识别开始，共 " + count + " 个文件")
    }
    ...
    function onGet(path, res) {
        ...
        if(res.code === 100) {
            results.push({
                path: path,
                data: res.data,
                invoice_type: res.invoice_type || "unknown",
                invoice_name: res.invoice_name || "未知发票"
            })
            // 显示结果
            resultsTableView.addInvoiceResult({
                title: path.split("/").pop().split("\\").pop(),
                invoiceType: res.invoice_name || "发票",
                content: formatInvoiceData(res.data)
            })
        }
    ...
    function onEnd(msg) {
        ...
        if(!msg || msg === "") {
            ...
            qmlapp.popup.simple(qsTr("发票识别完成"), errMsg, "success")
        }
    ...
    CtrlBar {
        ...
        CtrlBarButton {
            ...
            onClicked: {
                qmlapp.utilsConnector.selectFile(qsTr("选择发票图片"), "", [
        ...
        CtrlBarButton {
            ...
            onClicked: {
                qmlapp.utilsConnector.selectFile(qsTr("导出发票识别结果"), "", [
                    "Excel (*.xlsx)",
                    "CSV (*.csv)",
                    "JSON (*.json)"
                ], function(filePath) {
