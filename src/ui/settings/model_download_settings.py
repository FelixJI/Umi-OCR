...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QGroupBox, QPushButton, QTreeWidget, QTreeWidgetItem,
    QScrollArea, QDialog, QGridLayout, QMessageBox, QProgressBar
)
...
class ModelDownloadSettingsPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        for preset in MODEL_PRESETS.values():
            self.combo_preset.addItem(
                f"{preset.name} ({preset.total_size_mb}MB)", 
                preset.id
            )
        ...
        # 高级选择按钮
        self.btn_advanced = QPushButton("高级选择...")
        self.btn_advanced.clicked.connect(self._on_advanced_select)
        layout_download.addWidget(self.btn_advanced)
    ...
    def _on_advanced_select(self):
        dialog = ModelSelectionDialog(self)
        if dialog.exec() == QDialog.Accepted:
            if dialog.selected_models:
                self._start_download(dialog.selected_models)
    ...
    def _on_download_progress(self, model_name, current, total, speed):
        self.label_progress.setText(f"下载中: {model_name} - {current}/{total} ({speed:.1f} MB/s)")
