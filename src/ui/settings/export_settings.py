...
class ExportSettingsPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        current_fmt = self.controller.get_config(
            "export.default_format", OutputFormat.TXT.value
        )
        ...
        self.cb_copy.toggled.connect(
            lambda v: self.controller.set_config("export.auto_copy", v)
        )
        ...
        self.cb_conf.setChecked(
            self.controller.get_config("export.txt_with_confidence", False)
        )
        self.cb_conf.toggled.connect(
            lambda v: self.controller.set_config("export.txt_with_confidence", v)
        )
        ...
        self.spin_indent.valueChanged.connect(
            lambda v: self.controller.set_config("export.json_indent", v)
        )
        ...
        self.spin_quality.setValue(
            self.controller.get_config("export.pdf_image_quality", 90)
        )
        self.spin_quality.valueChanged.connect(
            lambda v: self.controller.set_config("export.pdf_image_quality", v)
        )
