...
class OcrSettingsPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        # 设置当前值
        current_engine = self.controller.get_config("ocr.engine_type", OcrEngineType.PADDLE.value)
        ...
        self.combo_lang.setCurrentText(self.controller.get_config("ocr.paddle.lang", "ch"))
        self.combo_lang.currentTextChanged.connect(lambda v: self.controller.set_config("ocr.paddle.lang", v))
        ...
        self.cb_gpu.toggled.connect(lambda v: self.controller.set_config("ocr.paddle.use_gpu", v))
        layout_paddle.addWidget(self.cb_gpu)
        
        self.cb_mkldnn = QCheckBox("使用 MKL-DNN 加速 (CPU)")
        self.cb_mkldnn.setChecked(self.controller.get_config("ocr.paddle.enable_mkldnn", True))
        self.cb_mkldnn.toggled.connect(lambda v: self.controller.set_config("ocr.paddle.enable_mkldnn", v))
        ...
        self.cb_denoise.setChecked(self.controller.get_config("ocr.preprocessing.enable_denoise", False))
        self.cb_denoise.toggled.connect(lambda v: self.controller.set_config("ocr.preprocessing.enable_denoise", v))
        layout_pre.addWidget(self.cb_denoise)
        
        # 二值化
        self.cb_binarize = QCheckBox("启用二值化")
        self.cb_binarize.setChecked(self.controller.get_config("ocr.preprocessing.enable_binarization", False))
        self.cb_binarize.toggled.connect(lambda v: self.controller.set_config("ocr.preprocessing.enable_binarization", v))
        layout_pre.addWidget(self.cb_binarize)
        
        # 纠偏
        self.cb_deskew = QCheckBox("启用纠偏")
        self.cb_deskew.setChecked(self.controller.get_config("ocr.preprocessing.enable_deskew", False))
        self.cb_deskew.toggled.connect(lambda v: self.controller.set_config("ocr.preprocessing.enable_deskew", v))
        ...
        self.cb_merge.toggled.connect(lambda v: self.controller.set_config("ocr.merge_lines", v))
