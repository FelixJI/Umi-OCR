...
VK_MAPPING = {
    ...
    'PAUSE': 0x13, 'CAPS LOCK': 0x14, 'ESC': 0x1B, 'ESCAPE': 0x1B,
    'SPACE': 0x20, 'PAGE UP': 0x21, 'PAGE DOWN': 0x22,
    'END': 0x23, 'HOME': 0x24, 'LEFT': 0x25, 'UP': 0x26,
    'RIGHT': 0x27, 'DOWN': 0x28, 'PRINT SCREEN': 0x2C,
...
class HotkeyManager(QObject):
    ...
    def register_hotkey(self, action_name: str, key_sequence_str: str) -> bool:
        ...
        success = self.user32.RegisterHotKey(None, hotkey_id, modifiers | MOD_NOREPEAT, vk_code)
        
        if success:
            self._action_map[hotkey_id] = action_name
            self._id_map[action_name] = hotkey_id
            logger.info(f"注册热键成功: {action_name} -> {key_sequence_str} (id={hotkey_id})")
            return True
        else:
            err_code = ctypes.get_last_error()
            logger.error(f"注册热键失败: {action_name} -> {key_sequence_str} (ErrorCode={err_code})")
            return False
        ...
        for part in parts:
            ...
            if part in ['CTRL', 'CONTROL']:
                ...
                if part in VK_MAPPING:
                    ...
                    ord_val = ord(part)
                    if (ord_val >= 48 and ord_val <= 57) or (ord_val >= 65 and ord_val <= 90):
                        vk_code = ord_val
                    else:
