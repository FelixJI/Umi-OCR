    ...
    if _Opt == "AA_UseOpenGLES":  # GLES需要检查，有些win7不支持
        if not QOpenGLContext.openGLModuleType() == QOpenGLContext.LibGLES:
            ...
            msg = "当前系统不支持OpenGLES，已禁用此渲染器。\n" \
                  ...
                  "The current system does not support OpenGLES and has disabled " \
                  "the program from using this renderer. \n" \
                  "If there are crashes or errors during this run, " \
                  "please restarting the program."
            logger.warning(msg)
...
# 方式三：使用 OpenGL ES 2.0 或更高版本，用d3d接口抽象成Opengl。
# 性能和效果都很好。但兼容性很差：
