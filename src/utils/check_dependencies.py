...
class OCRDependencyInfo:
    ...
    paddlepaddle: DependencyInfo = field(
        default_factory=lambda: DependencyInfo(
            name="PaddlePaddle",
            status=DependencyStatus.NOT_INSTALLED,
            description="深度学习框架",
        )
    )
    paddleocr: DependencyInfo = field(
        default_factory=lambda: DependencyInfo(
            name="PaddleOCR",
            status=DependencyStatus.NOT_INSTALLED,
            description="OCR识别引擎",
...
class DependencyChecker:
    ...
    def _check_paddlepaddle(self) -> None:
        ...
        try:
            ...
            if self._is_version_compatible(version, self.REQUIRED_PADDLE_VERSION):
                ...
                install_command = (
                    f"pip install paddlepaddle=={self.REQUIRED_PADDLE_VERSION}"
                )
    ...
    def _check_paddleocr(self) -> None:
        ...
        try:
            ...
            if self._is_version_compatible(version, self.REQUIRED_PADDLEOCR_VERSION):
                ...
                status = DependencyStatus.INCOMPATIBLE
                install_command = (
                    f"pip install paddleocr{self.REQUIRED_PADDLEOCR_VERSION}"
                )
    ...
    def _generate_recommendation(self) -> None:
        ...
        # 如果都已安装，不需要推荐
        if (
            self._paddlepaddle_info.status == DependencyStatus.INSTALLED
            and self._paddleocr_info.status == DependencyStatus.INSTALLED
        ):
    ...
    def is_ocr_available(self) -> bool:
        ...
        """
        return (
            self._paddlepaddle_info
            and self._paddlepaddle_info.status == DependencyStatus.INSTALLED
            and self._paddleocr_info
            and self._paddleocr_info.status == DependencyStatus.INSTALLED
        )
